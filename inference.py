    ...
    def inference_func(ckpt, dataset_name, evaluate_name):
        ...
        for video_name, video in videos_info.items():
            ...
            for i in range(num_his, length):
                ...
                if not os.path.isdir("Train_Data/" + video_name):
                    os.mkdir("Train_Data/" + video_name)
                
                im = Image.fromarray((sh[0,:,:,:]*255).astype(np.uint8))
                im.save("Train_Data/" + video_name + "/"  + str(i) + ".jpg")
                
                im2 = Image.fromarray((video_clip[...,-3:]*255).astype(np.uint8))
                im2.save("Train_Data/" + video_name + "/" + str(i) + "_gt"+".jpg")
